...
$IN  = "data\poland-251010.osm.pbf"
$TMP = "data\poland_full_data_filtred.pbf"
$OUT = "data\poland-poi-walk.pbf"
...
if (-not (Test-Path $OSMIUM)) {
  throw "Nie znaleziono osmium w aktywnym conda env. Użyj: conda activate osmtools"
}
...
$args1 = @(
  ...
  "nwr/highway=residential",
  "nwr/highway=unclassified",
  "nwr/highway=service",
...
$args2 = @("derive-ways", $TMP, "-o", $OUT, "-O", "-v")
Write-Host "`n> $OSMIUM $($args2 -join ' ')" -ForegroundColor Cyan
& $OSMIUM @args2
if ($LASTEXITCODE -ne 0) { throw "osmium derive-ways failed: exit $LASTEXITCODE" }
